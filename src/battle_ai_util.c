        ...
        for (i = 0; i < gMovesInfo[move].numAdditionalEffects; i++)
        {
            switch (gMovesInfo[move].additionalEffects[i].moveEffect)
            {
                ...
                case MOVE_EFFECT_ATK_DEF_DOWN:
                    if ((gMovesInfo[move].additionalEffects[i].self && GetBattlerAbility(battlerAtk) == ABILITY_HYPER_CUTTER))
                        break;
                // fallthrough
                case MOVE_EFFECT_DEF_MINUS_1:
                case MOVE_EFFECT_SPD_MINUS_1:
                ...
                case MOVE_EFFECT_SP_DEF_MINUS_1:
                case MOVE_EFFECT_SP_DEF_MINUS_2:
                    if ((gMovesInfo[move].additionalEffects[i].self && GetBattlerAbility(battlerAtk) == ABILITY_BIG_PECKS))
                        break;
...
bool32 IsMoveEncouragedToHit(u32 battlerAtk, u32 battlerDef, u32 move)
{
    ...
    if ((gMovesInfo[move].effect == EFFECT_TOXIC && IS_BATTLER_OF_TYPE(battlerAtk, TYPE_POISON))
        || (gMovesInfo[move].effect == EFFECT_WILL_O_WISP && IS_BATTLER_OF_TYPE(battlerAtk, TYPE_FIRE))
        || (move == MOVE_THUNDER_WAVE && IS_BATTLER_OF_TYPE(battlerAtk, TYPE_ELECTRIC)))
        return TRUE;

    // discouraged from hitting
    weather = AI_GetWeather(AI_DATA);
    if (((weather & B_WEATHER_SUN) || AI_DATA->abilities[battlerDef] == ABILITY_SUN_SALUTE) && gMovesInfo[move].effect == EFFECT_THUNDER)
        return FALSE;
...
bool32 ShouldLowerStat(u32 battler, u32 battlerAbility, u32 stat)
{
    if (gBattleMons[battler].statStages[stat] > MIN_STAT_STAGE && battlerAbility != ABILITY_CONTRARY)
    {
        if (AI_DATA->holdEffects[battler] == HOLD_EFFECT_CLEAR_AMULET
         ...
         || battlerAbility == ABILITY_FLOWER_VEIL)
            return FALSE;
...
bool32 CanKnockOffItem(u32 battler, u32 item)
{
    ...
    if (AI_DATA->abilities[battler] == ABILITY_STICKY_HOLD || AI_DATA->abilities[battler] == ABILITY_BALL_FETCH)
        return FALSE;
