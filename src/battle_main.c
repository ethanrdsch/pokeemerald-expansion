...
const struct TrainerClass gTrainerClasses[TRAINER_CLASS_COUNT] =
{
    ...
    [TRAINER_CLASS_BIRD_KEEPER] = { _("BIRD KEEPER"), 8, BALL_QUICK },
    ...
    [TRAINER_CLASS_AROMA_LADY] = { _("AROMA LADY"), 10, BALL_FRIEND },
    ...
    [TRAINER_CLASS_GUITARIST] = { _("GUITARIST"), 8, BALL_HEAVY },
    [TRAINER_CLASS_KINDLER] = { _("KINDLER"), 8 },
    [TRAINER_CLASS_CAMPER] = { _("CAMPER"), 4, BALL_SAFARI },
    ...
    [TRAINER_CLASS_PARASOL_LADY] = { _("PARASOL LADY"), 10, BALL_LOVE },
    [TRAINER_CLASS_SWIMMER_F] = { _("SWIMMER♀"), 2, BALL_DIVE },
    [TRAINER_CLASS_TWINS] = { _("TWINS"), 3 },
    [TRAINER_CLASS_SAILOR] = { _("SAILOR"), 8, BALL_LURE },
...
void SwitchInClearSetData(u32 battler)
{
    ...
    gBattleStruct->soundMoveTurns[battler] = 0;
...
u32 GetBattlerTotalSpeedStatArgs(u32 battler, u32 ability, enum ItemHoldEffect holdEffect)
{
    ...
    if (HasWeatherEffect())
    {
        if (ability == ABILITY_SWIFT_SWIM       && holdEffect != HOLD_EFFECT_UTILITY_UMBRELLA && gBattleWeather & B_WEATHER_RAIN)
            ...
            speed *= 2;
        else if (ability == ABILITY_SLUSH_RUSH  && (gBattleWeather & (B_WEATHER_HAIL | B_WEATHER_SNOW | B_WEATHER_HAILSTORM)))
            speed *= 2;
    }
    ...
    if (ability == ABILITY_QUICK_FEET && gBattleMons[battler].status1 & STATUS1_ANY)
        ...
        speed *= 2;
    else if (ability == ABILITY_FLOWER_GIFT && !(gBattleMons[battler].status2 & STATUS2_TRANSFORMED) && ((gBattleWeather & B_WEATHER_SUN && HasWeatherEffect())))
        speed *= 1.5;
                              ...
                              enum ItemHoldEffect holdEffectBattler1, enum ItemHoldEffect holdEffectBattler2, u32 speedBattler1, u32 speedBattler2, s32 priority1, s32 priority2)
{
    ...
    if (priority1 == priority2)
    {
        ...
        bool32 battler1HasQuickEffect = gProtectStructs[battler1].quickDraw || gProtectStructs[battler1].usedCustapBerry
                                        || (ability1 == ABILITY_STALL && gFieldStatuses & STATUS_FIELD_TRICK_ROOM && gFieldTimers.trickRoomTimer != gBattleTurnCounter);
        bool32 battler2HasQuickEffect = gProtectStructs[battler2].quickDraw || gProtectStructs[battler2].usedCustapBerry
                                        || (ability2 == ABILITY_STALL && gFieldStatuses & STATUS_FIELD_TRICK_ROOM && gFieldTimers.trickRoomTimer != gBattleTurnCounter);
        bool32 battler1HasStallingAbility = (ability1 == ABILITY_STALL && !(gFieldStatuses & STATUS_FIELD_TRICK_ROOM && gFieldTimers.trickRoomTimer != gBattleTurnCounter))
                                        || (ability1 == ABILITY_MYCELIUM_MIGHT && IsBattleMoveStatus(gChosenMoveByBattler[battler1]));
        bool32 battler2HasStallingAbility = (ability2 == ABILITY_STALL && !(gFieldStatuses & STATUS_FIELD_TRICK_ROOM && gFieldTimers.trickRoomTimer != gBattleTurnCounter))
                                        || (ability2 == ABILITY_MYCELIUM_MIGHT && IsBattleMoveStatus(gChosenMoveByBattler[battler2]));
...
u32 GetDynamicMoveType(struct Pokemon *mon, u32 move, u32 battler, enum MonState state)
{
            ...
            if (HasWeatherEffect())
            {
                if (gBattleWeather & B_WEATHER_RAIN && holdEffect != HOLD_EFFECT_UTILITY_UMBRELLA)
                    ...
                    return TYPE_ROCK;
                            ...
                            || GetBattlerAbility(battler) == ABILITY_SUN_SALUTE)
                    return TYPE_FIRE;
                else if (gBattleWeather & (B_WEATHER_SNOW | B_WEATHER_HAIL | B_WEATHER_HAILSTORM))
                    return TYPE_ICE;
                else
                    return moveType;
            }
    ...
    if (IsSoundMove(move) && ability == ABILITY_LIQUID_VOICE && moveType == TYPE_NORMAL)
    {
        return TYPE_WATER;
    }
    else if (IsSoundMove(move) && ability == ABILITY_GHASTLY_WAIL && moveType == TYPE_NORMAL)
    {
        return TYPE_FIRE;
    }
