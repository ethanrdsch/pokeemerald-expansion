...
static void BattleStartClearSetData(void)
{
    ...
    if (!(gBattleTypeFlags & BATTLE_TYPE_RECORDED))
    {
        if (!(gBattleTypeFlags & BATTLE_TYPE_LINK) && gSaveBlock2Ptr->optionsBattleSceneOff == 1)
            gHitMarker |= HITMARKER_NO_ANIMATIONS;
        else if (!(gBattleTypeFlags & BATTLE_TYPE_LINK) && gSaveBlock2Ptr->optionsBattleSceneOff == 2 && VarGet(VAR_FORCEANIM) != 1)
            gHitMarker |= HITMARKER_NO_ANIMATIONS;
    }
...
static void HandleTurnActionSelectionState(void)
{
    ...
    for (battler = 0; battler < gBattlersCount; battler++)
    {
        ...
        case STATE_WAIT_ACTION_CHOSEN: // Try to perform an action.
            ...
            #else
                if (!(gBattleTypeFlags & BATTLE_TYPE_TRAINER)
                    && gBattleBufferB[battler][1] == B_ACTION_RUN_PROMPT)
            #endif
                {
...
void SetTypeBeforeUsingMove(u32 move, u32 battlerAtk)
{
    ...
    // changed Raging Bull to use secondary type instead of Tauros breed
    ...
    if (gMovesInfo[move].type == TYPE_NORMAL
             ...
             && gMovesInfo[move].effect != EFFECT_REVELATION_DANCE
             && gMovesInfo[move].effect != EFFECT_RAGING_BULL
             && ((attackerAbility == ABILITY_PIXILATE && (ateType = TYPE_FAIRY))
                 || (attackerAbility == ABILITY_REFRIGERATE && (ateType = TYPE_ICE))
                 || (attackerAbility == ABILITY_AERILATE && (ateType = TYPE_FLYING))
                 || ((attackerAbility == ABILITY_GALVANIZE) && (ateType = TYPE_ELECTRIC))
                 || ((attackerAbility == ABILITY_FLORALIZE) && (ateType = TYPE_GRASS))
                 || ((attackerAbility == ABILITY_TERRALATE) && (ateType = TYPE_GROUND))
                )
             )
    {
