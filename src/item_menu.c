...
//static bool8 Register_IsItemInList(u16 itemId); <- unused
//static void Task_ScrollingMultichoiceInput(u8 taskId); <- unused
...
//static EWRAM_DATA u8 sRegisteredItemsMenuIcon = 0xFF; <- unused
...
{
    s16* data = gTasks[taskId].data;
    //u16* scrollPos = &gBagPosition.scrollPosition[gBagPosition.pocket]; <- apparently unused
    //u16* cursorPos = &gBagPosition.cursorPosition[gBagPosition.pocket]; <- apparently unused
...
{
    s16* data = gTasks[taskId].data;
    //u16* scrollPos = &gBagPosition.scrollPosition[gBagPosition.pocket]; <- unused
    //u16* cursorPos = &gBagPosition.cursorPosition[gBagPosition.pocket]; <- unused
...
static void SortItemsInBag(u8 pocket, u8 type)
{
    ...
    u16 itemAmount;
    //s8 (*func)(struct ItemSlot*, struct ItemSlot*); <- unused
