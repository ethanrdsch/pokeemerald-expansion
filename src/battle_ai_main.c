...
// the following checks apply to any target (including user)
    ...
    // other
        ...
        case EFFECT_AURORA_VEIL:
            if (gSideStatuses[GetBattlerSide(battlerAtk)] & SIDE_STATUS_AURORA_VEIL
              || PartnerHasSameMoveEffectWithoutTarget(BATTLE_PARTNER(battlerAtk), move, aiData->partnerMove)
              || !(weather & (B_WEATHER_HAIL | B_WEATHER_SNOW | B_WEATHER_HAILSTORM)))
                ADJUST_SCORE(-10);
            break;
        case EFFECT_OHKO:
        ...
        case EFFECT_FLATTER:
            if (!AI_CanConfuse(battlerAtk, battlerDef, aiData->abilities[battlerDef], BATTLE_PARTNER(battlerAtk), move, aiData->partnerMove)
                || aiData->abilities[battlerDef] == ABILITY_TANGLED_FEET)
                ADJUST_SCORE(-10);
            break;
        case EFFECT_SUBSTITUTE:
            if (gBattleMons[battlerAtk].status2 & STATUS2_SUBSTITUTE 
                || aiData->abilities[battlerDef] == ABILITY_INFILTRATOR
                || (aiData->abilities[battlerDef] == ABILITY_UNSEEN_FIST && HasMoveWithFlag(battlerDef, MoveMakesContact)))
                ADJUST_SCORE(-8);
        ...
        case EFFECT_MOONLIGHT:
            if ((AI_GetWeather() & (B_WEATHER_RAIN | B_WEATHER_SANDSTORM | B_WEATHER_HAIL | B_WEATHER_SNOW | B_WEATHER_FOG | B_WEATHER_HAILSTORM)))
                ADJUST_SCORE(-3);
        ...
        case EFFECT_RECOIL_IF_MISS:
            if (aiData->abilities[battlerAtk] != ABILITY_MAGIC_GUARD && aiData->abilities[battlerAtk] != ABILITY_IMPENETRABLE
        ...
        case EFFECT_SPICY_EXTRACT:
            if (battlerAtk != BATTLE_PARTNER(battlerDef)
             && (HasMoveWithCategory(battlerDef, DAMAGE_CATEGORY_PHYSICAL)
              ...
              || aiData->abilities[battlerDef] == ABILITY_WONDER_SKIN
    ...
    if (HOLD_EFFECT_CHOICE(aiData->holdEffects[battlerAtk]) && IsBattlerItemEnabled(battlerAtk))
    {
        ...
        && !(moveEffect == EFFECT_AURORA_VEIL && (AI_GetWeather() & (B_WEATHER_SNOW | B_WEATHER_HAIL | B_WEATHER_HAILSTORM))))
            ADJUST_SCORE(-30);
        ...
        if (!partnerProtecting)
        {
            ...
            case EFFECT_SWAGGER:
                if (gBattleMons[battlerAtkPartner].statStages[STAT_ATK] < MAX_STAT_STAGE
                 ...
                 && (!AI_CanBeConfused(battlerAtk, battlerAtkPartner, move, atkPartnerAbility)
                  ...
                  || atkPartnerAbility == ABILITY_TANGLED_FEET))
                {
                    RETURN_SCORE_PLUS(WEAK_EFFECT);
                }
                break;
            ...
            case EFFECT_SKILL_SWAP:
                if (aiData->abilities[battlerAtk] != aiData->abilities[BATTLE_PARTNER(battlerAtk)] && !attackerHasBadAbility)
                {
                    ...
                    if ((aiData->abilities[battlerAtk] == ABILITY_COMPOUND_EYES || aiData->abilities[battlerAtk] == ABILITY_AERIAL_SCOUT)
...
static u32 AI_CalcMoveEffectScore(u32 battlerAtk, u32 battlerDef, u32 move)
{
    ...
    // check waking moves
    if (gBattleMons[battlerAtk].status1 & STATUS1_SLEEP && MoveWakesUser(move))
        ADJUST_SCORE(10);
    ...
    case EFFECT_POPULATION_BOMB:
        if (AI_MoveMakesContact(aiData->abilities[battlerAtk], aiData->holdEffects[battlerAtk], move)
          ...
          && aiData->abilities[battlerAtk] != ABILITY_IMPENETRABLE
    ...
    case EFFECT_REST:
        ...
        else if (ShouldRecover(battlerAtk, battlerDef, move, 100, AI_DEFENDING))
        {
            if (aiData->holdEffects[battlerAtk] == HOLD_EFFECT_CURE_SLP
              ...
              || aiData->abilities[battlerAtk] == ABILITY_HEALER
    ...
    case EFFECT_FORESIGHT:
        if (aiData->abilities[battlerAtk] == ABILITY_SCRAPPY 
            ...
            || aiData->abilities[battlerAtk] == ABILITY_NORMALIZE)
            break;
    ...
    case EFFECT_BESTOW:
        ...
        case HOLD_EFFECT_BLACK_SLUDGE:
            if (!IS_BATTLER_OF_TYPE(battlerDef, TYPE_POISON) 
                && aiData->abilities[battlerDef] != ABILITY_MAGIC_GUARD
                && aiData->abilities[battlerDef] != ABILITY_IMPENETRABLE)
                ADJUST_SCORE(DECENT_EFFECT);
            break;
...
static s32 AI_PredictSwitch(u32 battlerAtk, u32 battlerDef, u32 move, s32 score)
{
    ...
    u32 aiRocksFlags = gSideStatuses[GetBattlerSide(battlerAtk)] & (SIDE_STATUS_STEALTH_ROCK);
    enum BattleMoveEffects moveEffect = GetMoveEffect(move);
    struct AiLogicData *aiData = gAiLogicData;
    uq4_12_t effectiveness = aiData->effectiveness[battlerAtk][battlerDef][gAiThinkingStruct->movesetIndex];
    u32 predictedMove = GetIncomingMove(battlerAtk, battlerDef, gAiLogicData);
