...
u32 GetTotalAccuracy(u32 battlerAtk, u32 battlerDef, u32 move, u32 atkAbility, u32 defAbility, u32 atkHoldEffect, u32 defHoldEffect)
{
    ...
    if ((IsBattlerWeatherAffected(battlerDef, B_WEATHER_SUN)
        || GetBattlerAbility(battlerDef) == ABILITY_SUN_SALUTE
        || GetBattlerAbility(battlerAtk) == ABILITY_SUN_SALUTE)
      && (gMovesInfo[move].effect == EFFECT_THUNDER))
        moveAcc = 50;
...
s32 CalcCritChanceStageArgs(u32 battlerAtk, u32 battlerDef, u32 move, bool32 recordAbility, u32 abilityAtk, u32 abilityDef, u32 holdEffectAtk)
{
    ...
    else
    {
        critChance  = 2 * ((gBattleMons[battlerAtk].status2 & STATUS2_FOCUS_ENERGY) != 0)
                    ...
                    + (abilityAtk == ABILITY_STARRY_FIST && gMovesInfo[gCurrentMove].punchingMove)
                    + gBattleStruct->bonusCritStages[gBattlerAttacker];

        // Record ability only if move had at least +3 chance to get a crit
        if (critChance >= 3 && recordAbility && (abilityDef == ABILITY_BATTLE_ARMOR 
...
void SetMoveEffect(bool32 primary, bool32 certain)
{
    ...
    if (gBattleScripting.moveEffect <= PRIMARY_STATUS_MOVE_EFFECT) // status change
    {
        ...
        case STATUS1_BURN:
            ...
            if ((battlerAbility == ABILITY_WATER_VEIL || battlerAbility == ABILITY_WATER_BUBBLE || battlerAbility == ABILITY_HEATPROOF)
        ...
        else
        {
            ...
            case MOVE_EFFECT_STEAL_ITEM:
                {
                        ...
                        && (gWishFutureKnock.knockedOffMons[side] & gBitTable[gBattlerPartyIndexes[gBattlerAttacker]]))
                    {
                        ...
                        && (GetBattlerAbility(gBattlerTarget) == ABILITY_STICKY_HOLD || GetBattlerAbility(gBattlerTarget) == ABILITY_BALL_FETCH))
                    {
                        BattleScriptPushCursor();
            ...
            case MOVE_EFFECT_BUG_BITE:
                if (ItemId_GetPocket(gBattleMons[gEffectBattler].item) == POCKET_BERRIES
                    ...
                    && battlerAbility != ABILITY_BALL_FETCH)
                {
            ...
            case MOVE_EFFECT_PSYCHIC_NOISE:
                ...
                else if (!(gStatuses3[gEffectBattler] & STATUS3_HEAL_BLOCK))
                {
                    ...
                    gDisableStructs[gEffectBattler].healBlockTimer = 3;
                    BattleScriptPush(gBattlescriptCurrInstr + 1);
...
static bool32 TryKnockOffBattleScript(u32 battlerDef)
{
    if (gBattleMons[battlerDef].item != 0
        ...
        && !NoAliveMonsForEitherParty())
    {
        if ((GetBattlerAbility(battlerDef) == ABILITY_STICKY_HOLD || GetBattlerAbility(battlerDef) == ABILITY_BALL_FETCH) 
        ...
        case MOVEEND_MAGICIAN:
            if (GetBattlerAbility(gBattlerAttacker) == ABILITY_MAGICIAN
              ...
              && (GetBattlerAbility(gBattlerTarget) != ABILITY_STICKY_HOLD 
                    || GetBattlerAbility(gBattlerTarget) != ABILITY_BALL_FETCH
            ...
            if (GetBattlerAbility(gBattlerAttacker) == ABILITY_FRISK
              && gCurrentMove != MOVE_FLING && gCurrentMove != MOVE_NATURAL_GIFT
              && gBattleMons[gBattlerAttacker].item == ITEM_NONE
              && gBattleMons[gBattlerTarget].item != ITEM_NONE
              && IsBattlerAlive(gBattlerAttacker)
              && TARGET_TURN_DAMAGED
              && gMovesInfo[gCurrentMove].makesContact
              && CanStealItem(gBattlerAttacker, gBattlerTarget, gBattleMons[gBattlerTarget].item)
              && !gSpecialStatuses[gBattlerAttacker].gemBoost   // In base game, gems are consumed after magician would activate.
              && !(gWishFutureKnock.knockedOffMons[GetBattlerSide(gBattlerTarget)] & gBitTable[gBattlerPartyIndexes[gBattlerTarget]])
              && !DoesSubstituteBlockMove(gBattlerAttacker, gBattlerTarget, gCurrentMove)
...
bool32 CanPoisonType(u8 battlerAttacker, u8 battlerTarget)
{
    return ((GetBattlerAbility(battlerAttacker) == ABILITY_CORROSION)
            || (gBattleMons[gBattlerAttacker].ability == ABILITY_MYCELIUM_MIGHT && gMovesInfo[gCurrentMove].category == DAMAGE_CATEGORY_STATUS)
            || !(IS_BATTLER_OF_TYPE(battlerTarget, TYPE_POISON) || IS_BATTLER_OF_TYPE(battlerTarget, TYPE_STEEL))
            || ((IS_BATTLER_OF_TYPE(battlerTarget, TYPE_STEEL)) && gMovesInfo[gCurrentMove].effect == EFFECT_POISON_STEEL_HIT));
}
...
static void Cmd_various(void)
{
    ...
    {
        VARIOUS_ARGS();
        if (gMovesInfo[gCurrentMove].effect == EFFECT_RECHARGE_EXCEPT_FAINT
            && HasAttackerFaintedTarget()
            && !NoAliveMonsForEitherParty()
            && gBattleMons[gEffectBattler].status2 == STATUS2_RECHARGE)
        {
    ...
    case VARIOUS_TRY_INSTRUCT:
    {
        ...
        if (move == MOVE_NONE || move == MOVE_UNAVAILABLE || MoveHasAdditionalEffectSelf(move, MOVE_EFFECT_RECHARGE)
         ...
         || IsDynamaxed(gBattlerTarget)
         || gMovesInfo[move].effect == EFFECT_RECHARGE_EXCEPT_FAINT)
        {
            gBattlescriptCurrInstr = cmd->failInstr;
        }
        else
...
static void Cmd_jumpifnopursuitswitchdmg(void)
{
    ...
    if (gChosenActionByBattler[gBattlerTarget] == B_ACTION_USE_MOVE
        ...
        && gBattleMons[gBattlerAttacker].ability != ABILITY_RUN_AWAY)
    {
        s32 i;
...
static bool32 CheckIfCanFireTwoTurnMoveNow(u8 battler, bool8 checkChargeTurnEffects)
{
    ...
    if (GetBattlerAbility(battler) == ABILITY_SUN_SALUTE
        && HIHALF(gMovesInfo[gCurrentMove].argument) == B_WEATHER_SUN)
        return TRUE;
    ...
    if (IsBattlerWeatherAffected(battler, HIHALF(gMovesInfo[gCurrentMove].argument))
        || IsBattlerTerrainAffected(battler, HIHALF(gMovesInfo[gCurrentMove].argument)))
        return TRUE;
...
void BS_JumpIfAbsorb(void)
{
    ...
    if (IsElectricAbilityAffected(ABILITY_VOLT_ABSORB)
        || IsElectricAbilityAffected(ABILITY_MINUS))
        gBattlescriptCurrInstr = cmd->jumpInstr;
    else
        gBattlescriptCurrInstr = cmd->nextInstr;
}
