...
void PrepareStringBattle(enum StringID stringId, u32 battler)
{
    ...
    // Check Defiant, Competitive, Vital Spirit stat raise whenever a stat is lowered.
              ...
              && ((targetAbility == ABILITY_DEFIANT && CompareStat(gBattlerTarget, STAT_ATK, MAX_STAT_STAGE, CMP_LESS_THAN))
                 ...
                 || (targetAbility == ABILITY_VITAL_SPIRIT && CompareStat(gBattlerTarget, STAT_SPEED, MAX_STAT_STAGE, CMP_LESS_THAN)))
...
u8 CheckMoveLimitations(u32 battler, u8 unusableMoves, u16 check)
{
    ...
    for (i = 0; i < MAX_MON_MOVES; i++)
    {
        ...
        // Stuff Cheeks
            ...
            unusableMoves |= 1u << i;
        // Gorilla Tactics
        else if (check & MOVE_LIMITATION_CHOICE_ITEM 
            && (GetBattlerAbility(battler) == ABILITY_GORILLA_TACTICS || GetBattlerAbility(battler) == ABILITY_SAGE_POWER)
...
static void CancellerTruant(u32 *effect)
{
    if (GetBattlerAbility(gBattlerAttacker) == ABILITY_TRUANT 
        ...
        && !(gMovesInfo[gCurrentMove].lazyMove))
    {
        CancelMultiTurnMoves(gBattlerAttacker, SKY_DROP_ATTACKCANCELLER_CHECK);
        gHitMarker |= HITMARKER_UNABLE_TO_USE_MOVE;
    ...
    else if (GetBattlerAbility(gBattlerAttacker) == ABILITY_TRUANT 
        && gDisableStructs[gBattlerAttacker].truantCounter
        && gMovesInfo[gCurrentMove].lazyMove)
    {
        gBattleCommunication[MULTISTRING_CHOOSER] = B_MSG_LOAFING;
        gBattlerAbility = gBattlerAttacker;
        BattleScriptPushCursor();
        gBattlescriptCurrInstr = BattleScript_TruantLoafingAroundLazyMove;
        *effect = 2;
    }
}
...
static void CancellerColorChange(u32 *effect)
{
    if (GetBattlerAbility(gBattlerTarget) == ABILITY_COLOR_CHANGE 
        && gBattlerAttacker != gBattlerTarget
        && GetBattleMoveCategory(gCurrentMove) != DAMAGE_CATEGORY_STATUS)
    {
        u32 moveType = GetBattleMoveType(gCurrentMove);
        u32 currentType;
        ...
        for (currentType = TYPE_NORMAL; currentType < NUMBER_OF_MON_TYPES; currentType++) 
        {
            ...
            if (currentModifier < bestModifier && currentModifier != UQ_4_12(0.0)) 
            {
                bestModifier = currentModifier;
                bestType = currentType;
            }
            if (bestModifier == UQ_4_12(0.5))
                break;
        }
...
static const MoveSuccessOrderCancellers sMoveSuccessOrderCancellers[] =
{
    ...
    [CANCELLER_COLOR_CHANGE] = CancellerColorChange,
...
bool32 CanAbilityBlockMove(u32 battlerAtk, u32 battlerDef, u32 abilityAtk, u32 abilityDef, u32 move, enum AbilityEffectOptions option)
{
    ...
    case ABILITY_RECEIVER:
        if (IsThrowMove(move) && !(MoveMakesContact(move)))
        {
            if (gBattleMons[battlerAtk].status2 & STATUS2_MULTIPLETURNS)
                gHitMarker |= HITMARKER_NO_PPDEDUCT;
            battleScriptBlocksMove = BattleScript_SoundproofProtected;
...
bool32 CanAbilityAbsorbMove(u32 battlerAtk, u32 battlerDef, u32 abilityDef, u32 move, u32 moveType, enum AbilityEffectOptions option)
{
    ...
    case ABILITY_LIGHTNING_ROD:
        if (B_REDIRECT_ABILITY_IMMUNITY >= GEN_5 && moveType == TYPE_ELECTRIC && GetBattlerMoveTargetType(battlerAtk, move) != MOVE_TARGET_ALL_BATTLERS)
        {
            ...
            u32 attack = gBattleMons[battlerDef].attack;
            u32 spAttack = gBattleMons[battlerDef].spAttack;

            attack = attack * gStatStageRatios[gBattleMons[battlerDef].statStages[STAT_ATK]][0];
            attack = attack / gStatStageRatios[gBattleMons[battlerDef].statStages[STAT_ATK]][1];

            spAttack = spAttack * gStatStageRatios[gBattleMons[battlerDef].statStages[STAT_SPATK]][0];
            spAttack = spAttack / gStatStageRatios[gBattleMons[battlerDef].statStages[STAT_SPATK]][1];
...
u32 AbilityBattleEffects(u32 caseID, u32 battler, u32 ability, u32 special, u32 moveArg)
{
    ...
    case ABILITYEFFECT_ON_SWITCHIN:
        ...
        case ABILITY_IMPOSTER:
            ...
            break;
        case ABILITY_SUN_SALUTE:
            if (!gSpecialStatuses[battler].switchInAbilityDone)
            {
                gBattleCommunication[MULTISTRING_CHOOSER] = B_MSG_SWITCHIN_SUN_SALUTE;
        ...
        case ABILITY_PASTEL_VEIL:
            if (!gSpecialStatuses[battler].switchInAbilityDone)
            {
                ...
                if (!(gSideStatuses[GetBattlerSide(gBattlerAttacker)] & SIDE_STATUS_SAFEGUARD))
                {
                    ...
                    BattleScriptPushCursorAndCallback(BattleScript_EffectSafeguardAbility);
                    effect++;
                }
        ...
        case ABILITY_PICKUP:
            if (!gSpecialStatuses[battler].switchInAbilityDone)
            {
                gSpecialStatuses[battler].switchInAbilityDone = TRUE;
                gBattlerAttacker = battler;
                BattleScriptPushCursorAndCallback(BattleScript_TryAbilityRemoveHazards); // Try activate
                effect++;
            }
            return effect; // Note: It returns effect as to not record the ability if Pickup does not activate.
        ...
        case ABILITY_WHITE_SMOKE:
            if (!gSpecialStatuses[battler].switchInAbilityDone)
            {
                gBattlerAttacker = battler;
                gSpecialStatuses[battler].switchInAbilityDone = TRUE;
                bool8 triggerAbility = FALSE;
                for (i = 0; i < gBattlersCount; i++)
                    triggerAbility = TryResetBattlerStatChanges(i) || triggerAbility;
                if (triggerAbility)
                    BattleScriptPushCursorAndCallback(BattleScript_EffectHazeAbility);
                    effect++;
                break;
            }
        ...
        case ABILITY_SUPERSWEET_SYRUP:
            ...
            break;
        case ABILITY_ALLURING_ODOR:
            if (!gSpecialStatuses[battler].switchInAbilityDone)
            {
                gBattlerAttacker = battler;
                gSpecialStatuses[battler].switchInAbilityDone = TRUE;
                BattleScriptPushCursorAndCallback(BattleScript_AlluringOdorActivates);
                effect++;
            }
            break;
        case ABILITY_MONSTER_MASH:
            if (!gSpecialStatuses[battler].switchInAbilityDone)
            {
                gBattlerAttacker = battler;
                gSpecialStatuses[battler].switchInAbilityDone = TRUE;
                BattleScriptPushCursorAndCallback(BattleScript_MonsterMashActivates);
                effect++;
            }
            break;
    ...
    case ABILITYEFFECT_ENDTURN:
        if (IsBattlerAlive(battler))
        {
            ...
            // Healer now also has Shed Skin's effect
    ...
    case ABILITYEFFECT_MOVE_END: // Think contact abilities.
        ...
        */
        ...
        case ABILITY_IRON_BARBS:
            ...
            break;
        case ABILITY_REFLECTIVE:
            if (!(gBattleStruct->moveResultFlags[gBattlerTarget] & MOVE_RESULT_NO_EFFECT)
             && IsBattlerAlive(gBattlerAttacker)
             && !gProtectStructs[gBattlerAttacker].confusionSelfDmg
             && IsBattlerTurnDamaged(gBattlerTarget)
             && GetBattlerHoldEffect(gBattlerAttacker, TRUE) != HOLD_EFFECT_PROTECTIVE_PADS
             && !(IsMoveMakingContact(gBattlerAttacker, gBattlerTarget, GetBattlerAbility(gBattlerAttacker), GetBattlerHoldEffect(gBattlerAttacker, TRUE), gCurrentMove))
    ...
    case ABILITYEFFECT_MOVE_END_ATTACKER: // Same as above, but for attacker
        ...
        case ABILITY_CONTACT_FREEZE:
            if (!(gBattleStruct->moveResultFlags[gBattlerTarget] & MOVE_RESULT_NO_EFFECT)
             ...
             && !CanBattlerAvoidContactEffects(gBattlerAttacker, gBattlerTarget, GetBattlerAbility(gBattlerAttacker), GetBattlerHoldEffect(gBattlerAttacker, TRUE), move)
             && IsBattlerTurnDamaged(gBattlerTarget) // Need to actually hit the target
             && RandomPercentage(RNG_CONTACT_FREEZE, 30))
            {
        ...
        case ABILITY_HIGH_VOLTAGE:
            if (!(gBattleStruct->moveResultFlags[gBattlerTarget] & MOVE_RESULT_NO_EFFECT)
             ...
             && IsBattlerTurnDamaged(gBattlerTarget) // Need to actually hit the target
             && GetBattleMoveType(gCurrentMove) == TYPE_ELECTRIC
             && RandomPercentage(RNG_HIGH_VOLTAGE, 20))
            {
                gBattleScripting.moveEffect = MOVE_EFFECT_BURN;
                PREPARE_ABILITY_BUFFER(gBattleTextBuff1, gLastUsedAbility);
                BattleScriptPushCursor();
        ...
        case ABILITY_PARASITIC_PLANT:
             ...
             && GetBattleMoveType(gCurrentMove) == TYPE_GRASS)
            {
                ...
                gStatuses3[gBattlerTarget] |= STATUS3_LEECHSEED;
                BattleScriptPushCursor();
                gBattlescriptCurrInstr = BattleScript_AbilityLeechSeed;
                effect++;
            }
            break;
        case ABILITY_GULP_MISSILE:
            if ((gBattleMons[gBattlerAttacker].species == SPECIES_CRAMORANT)
             && ((IsBattlerTurnDamaged(gBattlerTarget)
                ...
                || gCurrentMove == MOVE_MUDDY_WATER
                || gCurrentMove == MOVE_SLUDGE_WAVE
...
u32 IsAbilityPreventingEscape(u32 battler)
{
    ...
    for (u32 battlerDef = 0; battlerDef < gBattlersCount; battlerDef++)
    {
        ...
        if (ability == ABILITY_MAGNET_PULL && IS_BATTLER_OF_TYPE(battler, TYPE_STEEL))
            return battlerDef + 1;
        if (ability == ABILITY_HONEY_GATHER && (IS_BATTLER_OF_TYPE(battler, TYPE_BUG) || IS_BATTLER_OF_TYPE(battler, TYPE_GRASS)))
            return battlerDef + 1;
    }
...
bool32 CanSetNonVolatileStatus(u32 battlerAtk, u32 battlerDef, u32 abilityAtk, u32 abilityDef, enum MoveEffects effect, enum NonVolatileStatus option)
{
    ...
    case MOVE_EFFECT_PARALYSIS:
        ...
        else if (B_PARALYZE_ELECTRIC >= GEN_6 && IS_BATTLER_OF_TYPE(battlerDef, TYPE_ELECTRIC) && GetBattlerAbility(battlerAtk) != ABILITY_MYCELIUM_MIGHT)
        {
            battleScript = BattleScript_NotAffected;
        }
    ...
    case MOVE_EFFECT_BURN:
        ...
        else if (IS_BATTLER_OF_TYPE(battlerDef, TYPE_FIRE) && GetBattlerAbility(battlerAtk) != ABILITY_MYCELIUM_MIGHT)
        {
            battleScript = BattleScript_NotAffected;
        }
    ...
    case MOVE_EFFECT_FROSTBITE:
        ...
        else if ((IS_BATTLER_OF_TYPE(battlerDef, TYPE_ICE) && GetBattlerAbility(battlerAtk) != ABILITY_MYCELIUM_MIGHT) 
    ...
    if (abilityDef == ABILITY_COMATOSE
     ...
     || abilityDef == ABILITY_PURE_CRYSTAL)
    {
        abilityAffected = TRUE;
        battleScript = BattleScript_AbilityProtectsDoesntAffect;
    }
        ...
        case HOLD_EFFECT_FLINCH:
            {
                ...
                if (ability == ABILITY_SERENE_GRACE
                    || (ability == ABILITY_STARRY_FIST && IsPunchingMove(gCurrentMove))
                    || (ability == ABILITY_PINPOINT && IsPiercingMove(gCurrentMove)))
...
bool32 IsMoveMakingContact(u32 battlerAtk, u32 battlerDef, u32 abilityAtk, enum ItemHoldEffect holdEffectAtk, u32 move)
{
    ...
    else if (holdEffectAtk == HOLD_EFFECT_LEEK && IsSlicingMove(move))
    {
        RecordItemEffectBattle(battlerAtk, HOLD_EFFECT_LEEK);
        return FALSE;
    }
...
bool32 IsBattlerProtected(u32 battlerAtk, u32 battlerDef, u32 move)
{
    ...
    if (IsSideProtected(battlerDef, PROTECT_CRAFTY_SHIELD)
     ...
     && GetMoveEffect(move) != EFFECT_COACHING)
        ...
        isProtected = TRUE;
    else if (gProtectStructs[battlerDef].protected == PROTECT_SILK_TRAP)
        isProtected = TRUE;
...
static inline u32 CalcMoveBasePower(struct DamageCalculationData *damageCalcData, u32 abilityDef, u32 weather)
{
    ...
    case EFFECT_WEATHER_BALL:
        if (weather & B_WEATHER_ANY || GetBattlerAbility(battlerAtk) == ABILITY_SUN_SALUTE)
            basePower *= 2;
        break;
    case EFFECT_PURSUIT:
    ...
    case EFFECT_CRUSH_GRIP:
        basePower =  30 + (gBattleMons[battlerDef].hp * basePower / gBattleMons[battlerDef].maxHP);
        break;          
    ...
    case EFFECT_HEAT_CRASH:
        ...
        if (GetBattlerAbility(battlerAtk) == ABILITY_HEAVY_IMPACT)
            basePower = 120;
...
static inline u32 CalcAttackStat(struct DamageCalculationData *damageCalcData, u32 atkAbility, u32 defAbility, enum ItemHoldEffect holdEffectAtk, u32 weather)
{
    ...
    else if (atkAbility == ABILITY_STARRY_FIST && IsPunchingMove(move))
    {
        atkStat  = gBattleMons[battlerAtk].spAttack;
        atkStage = gBattleMons[battlerAtk].statStages[STAT_SPATK];
    }
    ...
    case ABILITY_MINUS:
        ...
        break;
    case ABILITY_FLOWER_GIFT:
        if (gBattleMons[battlerAtk].species == SPECIES_CHERRIM_SUNSHINE && IsBattlerWeatherAffected(battlerAtk, B_WEATHER_SUN) && IsBattleMoveSpecial(move))
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
        break;
    case ABILITY_HUSTLE:
        if (IsBattleMovePhysical(move))
...
static inline void MulByTypeEffectiveness(uq4_12_t *modifier, u32 move, u32 moveType, u32 battlerDef, u32 defAbility, u32 defType, u32 battlerAtk, bool32 recordAbilities)
{
        ...
        && mod == UQ_4_12(0.0))
    {
        ...
        && abilityAtk == ABILITY_DRAGONS_MAW
        && mod == UQ_4_12(0.0))
    {
        mod = UQ_4_12(1.0);
        if (recordAbilities)
        ...
        && abilityAtk == ABILITY_HIGH_VOLTAGE
        && mod == UQ_4_12(0.5))
    {
        mod = UQ_4_12(2.0);
        if (recordAbilities)
...
static inline uq4_12_t CalcTypeEffectivenessMultiplierInternal(u32 move, u32 moveType, u32 battlerAtk, u32 battlerDef, bool32 recordAbilities, uq4_12_t modifier, u32 defAbility)
{
    ...
    else if (moveType == TYPE_FIRE && defAbility == ABILITY_SUN_SALUTE)
    {
        modifier = UQ_4_12(0.0);
        if (recordAbilities && defAbility == ABILITY_SUN_SALUTE)
        {
            gBattleStruct->moveResultFlags[battlerDef] |= (MOVE_RESULT_DOESNT_AFFECT_FOE);
            gLastUsedAbility = ABILITY_SUN_SALUTE;
            gLastLandedMoves[battlerDef] = 0;
            gBattleStruct->missStringId[battlerDef] = B_MSG_AVOIDED_DMG;
            RecordAbilityBattle(battlerDef, ABILITY_SUN_SALUTE);
        }
    }
    else if (moveType == TYPE_GROUND && !IsBattlerGroundedInverseCheck(battlerDef, INVERSE_BATTLE, CHECK_IRON_BALL) && !(MoveIgnoresTypeIfFlyingAndUngrounded(move)))
    {
        ...
        if (recordAbilities && defAbility == ABILITY_AERIAL_SCOUT)
        {
            gBattleStruct->moveResultFlags[battlerDef] |= (MOVE_RESULT_MISSED | MOVE_RESULT_DOESNT_AFFECT_FOE);
            gLastUsedAbility = ABILITY_AERIAL_SCOUT;
            gLastLandedMoves[battlerDef] = 0;
            gBattleStruct->missStringId[battlerDef] = B_MSG_GROUND_MISS;
    ...
    if (moveType != TYPE_MYSTERY)
    {
        ...
        if ((modifier <= UQ_4_12(1.0)  &&  abilityDef == ABILITY_WONDER_GUARD)
         ...
         || (moveType == TYPE_ELECTRIC && (abilityDef == ABILITY_LIGHTNING_ROD // TODO: Add Gen 3/4 config check
                                       ...
                                       || abilityDef == ABILITY_MINUS
                                       || abilityDef == ABILITY_MOTOR_DRIVE))
         || (moveType == TYPE_GHOST    &&  abilityDef == ABILITY_SOUL_ABSORB))
        {
            modifier = UQ_4_12(0.0);
        }
    }
...
{
    if ((weatherFlags & B_WEATHER_SUN) && GetBattlerAbility(battler) == ABILITY_SUN_SALUTE)
    {
        return TRUE;
    }
...
u32 CalcSecondaryEffectChance(u32 battler, u32 battlerAbility, const struct AdditionalEffect *additionalEffect)
{
    bool8 hasSereneGrace = (battlerAbility == ABILITY_SERENE_GRACE
                            || (battlerAbility == ABILITY_STARRY_FIST && IsPunchingMove(gCurrentMove))
                            || (battlerAbility == ABILITY_PINPOINT && IsPunchingMove(gCurrentMove)));
